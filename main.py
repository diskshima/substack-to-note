...
def main(in_dir: str, out_dir: str):
    ...
    for post in posts:
        ...
        if os.path.exists(html_file):
            ...
            etree.SubElement(item, WP + "post_date").text = etree.CDATA(
                to_wp_date(post_date_ja)
            )
            etree.SubElement(item, WP + "post_date_gmt").text = etree.CDATA(
                to_wp_date(post_date)
            )
            etree.SubElement(item, WP + "post_modified").text = etree.CDATA(
                to_wp_date(post_date_ja)
            )
            etree.SubElement(item, WP + "post_modified_gmt").text = etree.CDATA(
                to_wp_date(post_date)
            )
    ...
    wxr_string = etree.tostring(
        wxr_root, pretty_print=True, xml_declaration=True, encoding="UTF-8"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--in-dir", help="Input directory containing Substack export files"
    )
    parser.add_argument(
        "-o", "--out-dir", help="Output directory for WordPress WXR file and images"
    )
